...
Processor::Processor() {
    ...
    read_permissions.assign(CommandsVectorSize, false);
    write_permissions.assign(CommandsVectorSize, false);

    // Чтение доступно для команд вывода, загрузки в регистр и стек, переходов
    read_permissions[code_ops::out] = true;
    read_permissions[code_ops::outi] = true;
    read_permissions[code_ops::outf] = true;
    read_permissions[code_ops::outp] = true;

    read_permissions[code_ops::loadaddr] = true;
    read_permissions[code_ops::pushstack] = true;

    read_permissions[code_ops::jmp] = true;
    read_permissions[code_ops::jz] = true; // res == 0
    read_permissions[code_ops::jnz] = true; // res != 0
    read_permissions[code_ops::jgz] = true; // res >= 0
    read_permissions[code_ops::jlz] = true; // res <= 0
    read_permissions[code_ops::jg] = true; // res > 0
    read_permissions[code_ops::jl] = true; // res < 0

    read_permissions[code_ops::call] = true;

    // Запись -- для выталкивания и снятия вершины, ввода
    write_permissions[code_ops::in] = true;
    write_permissions[code_ops::ini] = true;
    write_permissions[code_ops::inf] = true;
    write_permissions[code_ops::inp] = true;

    write_permissions[code_ops::popstack] = true;
    write_permissions[code_ops::peekstack] = true;
...
void Processor::Execute()
{
    ...
    while (firstByte != 0)
    {
        ...
        // Здесь проверка условий отладки
        bool interrupt = false;
        for (size_t i = 0; i < 4; ++i)
        {
            auto arg = (address) loadData(sizeof(address), static_cast<address>(getIP() + 1));

            if ((DR_PROP[i].mode_ & BP_FLAGS::EXEC) && (DR_PROP[i].active_) && (DR[i] == getIP()))
                interrupt = true;
            if ((DR_PROP[i].mode_ & BP_FLAGS::READ) && (DR_PROP[i].active_) && (DR[i] == arg) && (find(begin(read_permissions), end(read_permissions), firstByte) != end(read_permissions)))
                interrupt = true;
            if ((DR_PROP[i].mode_ & BP_FLAGS::WRITE) && (DR_PROP[i].active_) && (DR[i] == arg) && (find(begin(write_permissions), end(write_permissions), firstByte) != end(write_permissions)))
                interrupt = true;
        }
        if (interrupt) break;
...
void Processor::Trace() {
    _log_trace << "IP = " << getIP() << " SP = " << (int)getSP() << " ZF = " << (int)getZF() << " SF = " << (int)getSF() << '\n';
    _log_trace << "Command code " << memory_[getIP()] << " size = " << getCommandSize(memory_[getIP()]) << '\n';
}
    ...
    while (!ifs.eof())
    {
        ...
        switch (prefix) {
            case 'c':
                ...
                if (getCommandSize((uint8_t)code_op) == CommandSizes::ThreeBytes) {
                    ifs >> adr;
